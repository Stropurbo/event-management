    ...
    if created:
        ...
        if assigned_email:
            try:
                ...
                print(f"✅ Email sent successfully to: {assigned_email}")
...
def notify_participants_on_addition(sender, instance, action, **kwargs):
    if action == 'post_add':
        participants = instance.participants.all()
        assigned_email = [p.email for p in participants]
        ...
        if assigned_email:
            ...
            print("⚠️ No participants found to notify.")


@receiver(post_delete, sender=Event)
def delete_event(sender, instance, **kwargs):
    print(f"🗑️ Event '{instance.name}' deleted successfully!")